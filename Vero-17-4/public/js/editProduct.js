document.addEventListener('DOMContentLoaded', () => {
  ...
  const imagesToRemoveHidden = document.getElementById('imagesToRemoveHidden');
   ...
   * `id` is a stable unique key — never shifts on removal.
   */
  let sizes = [];
   ...
   * Fixes the bug where splice shifted indices and deleted wrong items.
   */
  const removeSize = (id) => {
  ...
  // ─────────────────────────────────────────────
  // Pre-populate existing sizes from server data
  // ─────────────────────────────────────────────
  if (window.__existingSizes && Array.isArray(window.__existingSizes)) {
    window.__existingSizes.forEach((entry) => {
      if (typeof entry === 'object' && entry !== null && entry.size) {
        sizes.push({
          id: generateId(),
          size: entry.size,
        ...
        sizes.push({ id: generateId(), size: entry.trim(), quantity: 1 });
      }
    });
    renderSizes();
  }
  ...
  // ─────────────────────────────────────────────
  // Image removal checkboxes → hidden field
  // ─────────────────────────────────────────────
  if (editProductForm) {
    editProductForm.addEventListener('submit', () => {
      const checkedBoxes = document.querySelectorAll('.image-remove-checkbox:checked');
      const filenames = [];
    ...
    editProductForm.addEventListener('submit', (e) => {
      ...
      if (totalSize > 5 * 1024 * 1024) {
        e.preventDefault();
        alert('مجموع كل الصور يجب ألا يتجاوز 5 ميجابايت (Total images size must not exceed 5MB)');
      }
    });
  }
});
